...
public class SignerBESTest extends SignerTestBase {
	...
	XadesSigner signer = (SignerBES) new XadesBesSigningProfile(keyingProviderMy).newSigner();
	...
	//AnonymousDataObjectReference ref = new AnonymousDataObjectReference(new FileInputStream("./src/test/xml/document.xml"));
	...
	outputDocument(doc, "detached-2.xml");
    }

    @Ignore("")
    @Test
    public void testMySignBES() throws Exception {
	...
	DataObjectDesc obj1 = new DataObjectReference('#' + elemToSign.getAttribute("Id")).withTransform(new EnvelopedSignatureTransform()).withDataObjectFormat(
		new DataObjectFormatProperty("text/xml", "MyEncoding").withDescription("Isto é uma descrição do elemento raiz").withDocumentationUri("http://doc1.txt").withDocumentationUri("http://doc2.txt").withIdentifier("http://elem.root")).withCommitmentType(commitment).withDataObjectTimeStamp(dataObjsTimeStamp);
	...
	outputDocument(doc1, "document.signed.bes-2.xml");
    }
	...
	try {
	    new SignerBESTest().testSignBES();
	} catch (Exception ex) {
	    Logger.getLogger(SignerBESTest.class.getName()).log(Level.SEVERE, null, ex);
	}
    }
}
